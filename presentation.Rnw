...
data.frame(n_cmp = 1L:6) %>% 
  ...
  scale_y_continuous("Type I error probability\n(significance level 0.05)", breaks = c(1L:3/4, 0.95)) +
  my_theme
@


Multiple comparison problem: the more comparisons, the higher change of making the type I error.

\end{frame}

\begin{frame}{Comparison of multiple runs}
Solution: False Discovery Rate~\citep{benjamini_controlling_1995} (correction for multiple comparisons).
...
Comparison of multiple runs is one of the most common and crucial tasks in dPCR, but no software does that for you.
